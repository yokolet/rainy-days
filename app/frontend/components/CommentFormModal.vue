...
import { useCommentCreate } from "../composables/useCommentCreate"
...
const { body, commentCreate, commentCreateLoading, commentCreateError } =
  useCommentCreate(props.postId, props.replyId)
  ...
  <Teleport defer to="#app">
    <Transition>
      <section id="commentFormModal" v-show="props.isOpen">
        <div
          @click="$emit('close')"
          class="fixed inset-0 bottom-0 z-0 bg-stone-500 opacity-70
            dark:bg-stone-700"
        ></div>
        <div
          class="fixed inset-0 top-0 mx-auto my-auto h-fit w-4/5 rounded-xl
            bg-[#eff5f3] p-3 opacity-90 shadow-lg dark:bg-[#2e3231]"
        >
          <div class="w-full font-roboto-mono tracking-tight">
            ...
            >
              <h2 class="py-4 text-2xl font-bold">New Comment</h2>
              <label for="#content-input" class="block w-11/12">
                <span class="font-semibold">Comment</span>
                <span class="text-sm text-gray-600 dark:text-gray-400"
                  >1-300 characters</span
                >
                <textarea
                  id="content-input"
                  minlength="1"
...
.button-style {
  @apply block w-auto rounded-md border-[1px] border-white px-4 py-2 transition duration-150 hover:-translate-y-0.5 dark:border-stone-400;
}
